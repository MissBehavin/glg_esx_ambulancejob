...
RegisterNetEvent("esx_ambulancejob:missionText")
AddEventHandler("esx_ambulancejob:missionText", function(text, time)
        ClearPrints()
...
RegisterNetEvent('esx_ambulancejob:checkPulse')
AddEventHandler('esx_ambulancejob:checkPulse', function()
	local playerPed = GetPlayerPed(-1)
		TriggerClientEvent('esx_ambulancejob:missionText', 'The medic is checking your pulse. Is it ~g~Strong~w~, ~o~Weak~w~, or ~r~Nonexistant~w~?', 5000)
		Wait(5000)
		TriggerClientEvent('esx_ambulancejob:missionText', 'Please answer with /do <answer>', 5000)
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsHandcuffed then
	  ...
	  DisableControlAction(0,73,true) -- handcuff
	  DisableControlAction(0,243,true) -- handsup
	  DisableControlAction(0,244,true) -- radio
	  DisableControlAction(0,178,true) -- radio
	  DisableControlAction(0,167,true) -- F6
	  DisableControlAction(0,166,true) -- radio
	  DisableControlAction(0,168,true) -- radio
	  DisableControlAction(0,169,true) -- radio
	  DisableControlAction(0,170,true) -- radio
	  DisableControlAction(0,288,true) -- radio
	  DisableControlAction(0,289,true) -- radio
...
AddEventHandler('esx_ambulancejob:drag', function(MC)
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
      if IsDragged then
        local ped = GetPlayerPed(GetPlayerFromServerId(MCPed))
...
AddEventHandler('esx_ambulancejob:OutVehicle', function(t)
			...
			if data.current.value == 'citizen_interaction' then
				...
				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'citizen_interaction',
					{
						...
						{label = _U('ems_menu_fix'), 				value = 'fix_vehicle'}
						}
					},
					function(data, menu)
						...
						if data.current.value == 'handcuff' then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								ESX.ShowNotification(('You have handcuffed ') .. GetPlayerName(closestPlayer))
							end
						end
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('ems_clothes_ems'), value = 'EMS'},
			},
		},
		function(data, menu)
		...
		ESX.UI.Menu.Open(
			'default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				elements = {
					...
					{label = _U('EMStruck'), value = 'firepickup'},
					{label = _U('EMSsuv'), value = 'f'},
					{label = _U('EMSCharger'), value = 'fcharger'},					
				},
			},
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
					...
					if model == 'valkyrie' then
						SetVehicleModKit(vehicle, 0)
						SetVehicleLivery(vehicle, 1)
					end
					
					if model == 'firepickup' then
						SetVehicleModKit(vehicle, 0)
						SetVehicleLivery(vehicle, 1)
					end	
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' and (GetGameTimer() - GUI.Time) > 150 then
